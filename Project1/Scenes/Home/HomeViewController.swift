...
extension HomeViewController: UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        cell.doubleTapBlock = { [weak self] in
            self?.abc(indexPath: indexPath)
        }
        return cell
    }
    
    func abc(indexPath: IndexPath) {
        if let index = arrayIndex.firstIndex(of: indexPath.row) {
            let pre = arrayIndex[index]
            arrayIndex.remove(at: index)
            self.albumCollectionView.reloadItems(at: [.init(row: pre, section: 0)])
            for idx in index ..< arrayIndex.count {
                self.albumCollectionView.reloadItems(at: [.init(row: arrayIndex[idx], section: 0)])
            }
        } else {
